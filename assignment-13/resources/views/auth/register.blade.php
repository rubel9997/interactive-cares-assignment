<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="mt-4">
            ...
            <select id="vaccine_center_id" name="vaccine_center_id" class="block mt-1 w-full" required  autocomplete="vaccine_center_id">
                <option value="" selected disabled>Select a Vaccine Center</option>
                @foreach($vaccineCenters as $vaccineCenter)
                    <option value="{{ $vaccineCenter->id }}" {{ old('vaccine_center_id') == $vaccineCenter->id ? 'selected' : '' }}>
                        {{ $vaccineCenter->name }}
                    </option>
                @endforeach
            </select>
